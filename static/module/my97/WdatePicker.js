...
(function() {
    ...
    A = $.$dpPath || J();
    if ($.$wdate) K("./resource/module/my97/skin/WdatePicker.css");
    V = Y;
    if ($.$crossFrame) {
        try {
    ...
    function I(G, _) {
        ...
        if (!$dp.dd || G.eCont || ($dp.dd && (G.getRealLang().name != $dp.dd.lang || G.skin != $dp.dd.skin))) {
            if (G.eCont) E(G.eCont, G);
                ...
                //me fixed
                V[N].body.insertBefore($dp.dd, V[N].body.firstChild);
                E($dp.dd, G);
