...
var dialog = $.widget( "ui.dialog", {
	...
	_focusTabbable: function() {
        ...
        // 产品要求去掉定位表单元素
        this.uiDialogButtonPane.find(":tabbable").last().focus();
        return ;

        var tagName = hasFocus[0].tagName.toUpperCase();
			...
			this._on( this.document, {
				focusin: function( event ) {
					...
					if ( !this._allowInteraction( event ) ) {
						event.preventDefault();
						// Me Fix: 与wSelect 组件冲突
						// this._trackingInstances()[ 0 ]._focusTabbable();
					}
				}
			});
		}
		// Me Fix: 新的ui-front的z-index必须大于>= 现有ui-front最大的z-index+2
		var zIndicies = $( ".ui-front:visible" ).map(function() {
				return +$( this ).css( "z-index" );
			}).get();
		var zIndexMax = 100;
		if( zIndicies.length > 0 ) {
			zIndexMax = Math.max.apply( null, zIndicies ) + 2;
		}
		// Me Fix		
		if (/msie 6/i.test(navigator.userAgent)) {
			this.overlay = $( "<div>" )
				.addClass( "ui-widget-overlay ui-front" ).css({
                    width: $("#doc").width(),
